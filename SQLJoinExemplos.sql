...
SELECT tab1.col1, tab1.col2, tab2.col2, tab2.col4
FROM tab1, tab2
WHERE tab1.pk = tab2.fk AND condições
...
SELECT tab1.col1, tab1.col2, tab2.col2, tab2.col4, tab3.col2, tab3.col3
FROM tab1, tab2, tab3
WHERE tab1.pk = tab2.fk AND tab2.pk = tab3.fk AND condições
...
--EXEMPLO
--LISTA DE CHAMADA (RA COM - antes do ultimo digito)
--ORDENADO POR NUMERO, CASO O NOME > 30 caracteres mostrar o ultimo com . no final
--USANDO SQL2
SELECT SUBSTRING(a.ra, 1, 9)+'-'+SUBSTRING(a.ra, 10, 1) AS ra,
CASE WHEN (len(a.nome)>=30) THEN SUBSTRING(a.nome, 1, 29)+'.' ELSE a.nome END AS nome,
m.nome AS nome_materia
FROM alunos a INNER JOIN alunomateria am
ON a.ra = am.ra_aluno
INNER JOIN materias m
ON m.id = am.id_materia
WHERE m.nome LIKE 'Banco%' ORDER BY a.nome ASC
...
WHERE a.ra = am.ra_aluno AND m.id = am.id_materia AND m.nome LIKE 'Banco%'
ORDER BY a.nome ASC

--PEGAR AS NOTAS DA TURMA
--SQL2
SELECT SUBSTRING(a.ra, 1, 9)+'-'+SUBSTRING(a.ra, 10,1), a.nome, m.nome as nome_materia, av.tipo, av.peso, n.nota
